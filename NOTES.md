...
当前代码已经完成到:卡通表盘 + PCF85063 RTC + 局部刷新 + WiFiManager 配网 + NTP 自动校时 + 触摸卡通随机播放语音 + 菜单电量显示。代码已通过 PlatformIO 编译,但触摸/音频/电量读数还需要烧录到板子做硬件验证。
...
- 关机:运行中长按 PWR 约 2 秒,先在右上角画电源图标终帧,再释放 `BAT_Control` 断电
...
- 触摸:FT6336 已接入,点击卡通区域触发一次事件
- 音频:ES8311 + I2S 已接入,点击卡通时随机播放 `干什么.wav` 或 `不要打扰噜噜.wav`
- 菜单:单击 BOOT 进入四宫格菜单;再次单击 BOOT 从菜单/子页面回首页
- 电量:菜单第四格显示电池图标、百分比;测试阶段额外显示电池电压
...
│   ├── menu_screen.h/.cpp      <- BOOT 菜单 / 四宫格 / 音量页 / WiFi 页渲染
│   ├── touch_ft6336.h/.cpp     <- FT6336 触摸读取 + tap 事件
│   ├── audio_player.h/.cpp     <- ES8311 + I2S 播放内嵌 WAV + 音量控制
│   ├── generated/voice_assets.*<- 自动生成,两个语音 WAV 的 C 数组
...
防鬼影震荡:全黑 -> 全白 -> 全白
  |
  v
FT6336 touch init + ES8311/I2S audio init
  |
  v
...
开机 NTP:
  ...
  v
loop:
  - 长按 PWR 约 2 秒 -> 画电源图标终帧 -> 释放 BAT_Control 关机
  - 首页单击 BOOT -> 进入四宫格菜单
  - 菜单/子页面单击 BOOT -> 回首页
  - 首页触摸卡通区域 -> 随机播放一段语音,播放中再次触摸不叠音
  - 菜单第一格 WiFi -> 打开非阻塞 WiFiManager portal
  - 菜单第二格 Touch -> 开关首页触摸芯片/卡通交互,只局刷该格
  - 菜单第三格 Volume -> 进入音量页,左减右加,中间试听
  - 菜单第四格 Battery -> 显示电池百分比;测试阶段显示原始换算电压
...
| ES8311 codec | 0x18 | SDA=47, SCL=48 |

### 触摸 + 音频播放

触摸芯片是 FT6336,音频芯片是 ES8311。两者都在 Waveshare 这块板子的板载外设里,不需要额外接线。
...
- 两个 WAV 都是 48 kHz / 16-bit / mono PCM,和当前 I2S 播放参数直接匹配
- 两个 WAV 合计约 252 KB,放进 8MB Flash 没压力
- MP3 虽然更小,但要引入解码器和缓冲,会增加代码复杂度和实物调试变量

当前语音资源:

| 文件 | 格式 | 用途 |
|---|---|---|
| `assets/voice/干什么.wav` | 48 kHz,16-bit,mono PCM | 点击卡通随机播放之一 |
| `assets/voice/不要打扰噜噜.wav` | 48 kHz,16-bit,mono PCM | 点击卡通随机播放之一 |
| `assets/voice/*.mp3` | 48 kHz,mono MP3 | 暂不使用,作为源素材保留 |
...
| ES8311 MCLK | GPIO14 |
| ES8311 BCLK | GPIO15 |
| ES8311 LRCK/WS | GPIO38 |
| ES8311 DOUT | GPIO45 |
| Audio_PWR | GPIO42(active-low) |
| PA_CTRL | GPIO46 |
...
实测结论:
- `render_home_soft_clean()` 是“全屏白色 partial 一次 + 全屏主页 partial 一次”。用户实测:肉眼上和直接返回主页几乎没有区别,但当前保留为返回主页路径。
- 不要再尝试 SSD1681 `0x22` 控制字猜测中间刷新档。已经试过 `0xcf`、`0xdc`、`0xf4`,结果是:没有减少残影,还引入新的显示问题。这条路线已经判死刑。
- 后续如果继续优化残影,不要重复查询/尝试 `0xcf`/`0xdc`/`0xf4`/Mode2CF/Mode2DC/Mode2F4。更值得做的是查 Waveshare 官方 BSP/LUT 或重新设计页面切换策略。
...
- E-paper 断电后会保留最后画面,所以关机状态可以靠最后一帧判断
- 当前做法:长按 PWR 约 2 秒触发 `shutdown_now()`
- 关机流程:关 WiFi -> 关闭音频 -> `render_home_powered_off()` 右上角画电源图标 -> 关屏幕电源 -> 拉低 GPIO17 `BAT_Control`
- USB 插着时拉低 `BAT_Control` 不会真正断整板电源,所以代码随后进入 deep sleep;纯电池供电时会直接断电

### 11. 电池电量来自 GPIO4 ADC

- Waveshare 这块板子的电池采样接在 ESP32-S3 GPIO4 / ADC1_CH3
- 板上是 200K/200K 分压,所以代码里用 `VBAT = analogReadMilliVolts(GPIO4) * 2` 还原电池电压
- 当前百分比是简单线性估算:`3.3V = 0%`,`4.2V = 100%`;实物上如果不准,后面再换成锂电池放电曲线或查表
- 菜单第四格会显示电池图标和百分比;测试阶段还会显示一行电压,例如 `3.60V`
- release 时如要隐藏电压测试行,把 `src/menu_screen.cpp` 里的 `SHOW_BATTERY_DEBUG_VOLTAGE` 改成 `false`

### 12. daily NTP 不应该打开配网 portal

- 开机允许配网,因为用户就在旁边
- 每日 03:00 校时不能打开 portal,否则无网时会卡在 AP 配网等待
- 当前做法:`portal_timeout_s=0` 时 `WiFiManager.setEnableConfigPortal(false)`

### 13. 语音素材优先用 WAV,不是 MP3

- MP3 文件小,但 ESP32 播放 MP3 需要解码器、任务栈、缓冲和更多异常处理
- 当前两段 WAV 已经是 48 kHz mono PCM,可以直接把单声道样本复制成左右声道写入 I2S
- Flash 占用仍然很低,所以先用 WAV 更稳
...
- [ ] 正常开机后,长按 PWR 约 2 秒
- [ ] 屏幕右上角出现电源图标小标记
- [ ] 纯电池供电时,松手后设备停止刷新,最后画面保持在关机图标状态
- [ ] 再次长按 PWR 约 1-2 秒,设备重新开机并恢复正常表盘
- [ ] USB 插着时不会物理断电,但会进入 deep sleep,表现为不再更新屏幕
...
- [ ] 开机时 monitor 看到 `[touch] FT6336 ready`
- [ ] 开机时 monitor 看到 `[audio] ES8311 id=...` 和 `[audio] ready`
- [ ] 点击卡通区域,monitor 看到 `[touch] cartoon tap at x,y`
- [ ] 同一次点击后看到 `[audio] play: ...`
- [ ] 每次点击随机播放 `干什么` 或 `不要打扰噜噜`
- [ ] 播放中连续点击不会叠加多个声音
- [ ] 长按 PWR 关机时没有持续噪声,音频电源会被关闭

### BOOT 菜单验证

- [ ] 首页单击 BOOT,进入四宫格菜单
- [ ] 菜单再单击 BOOT,回到首页
- [ ] 点 WiFi 格,屏幕进入 WiFi 页,手机能看到 `PaperAssist-AP`
- [ ] WiFi 页单击 BOOT,portal 停止并回首页
- [ ] 点 Touch 格,该格局部刷新并在 `Touch ON/OFF` 之间切换
- [ ] Touch OFF 后回首页,FT6336 停止识别触摸且点卡通不播放;Touch ON 后恢复播放
- [ ] 点 Volume 格进入音量页,左侧降低、右侧提高、中间播放测试语音
- [ ] 第四格显示电池图标、百分比和测试电压;串口看到 `[menu] enter, battery=...mV ...%`
- [ ] 如果 release 要隐藏电压测试行,把 `SHOW_BATTERY_DEBUG_VOLTAGE` 改为 `false` 后重新编译
...
| 最高 | 烧录并验证触摸卡通随机播放语音 | 10-20 分钟 | 当前代码已编译通过,需要确认 ES8311 初始化和触摸坐标方向 |
| 高 | 优化 partial LUT | 2-3 小时 | 如果实物上灰雾明显再做 |
| 中 | 接 SHTC3 温湿度 | 1 小时 | 卡通脚下小字显示 `°C / RH%` |
| 中 | 扩展 FT6336 触控 | 1-2 小时 | 点屏切表盘风格 / 显示更多状态 |
| 低 | 校准电池百分比曲线 | 30-60 分钟 | 当前已显示,但百分比还是 3.3V-4.2V 线性估算 |
| 低 | PWR 按键长按深睡眠 | 1 小时 | 真正做低功耗时再推进 |

最后一句:任何涉及卡通图像的改动,先看 PNG 预览,满意再烧录。任何涉及时间源的改动,先看串口日志里的 `[clock]`、`[rtc]`、`[net]`、`[ntp]` 四类输出。任何涉及触摸/音频的改动,先看 `[touch]` 和 `[audio]` 日志,确认硬件有响应再调体验。
