...
- 关机:运行中长按 PWR 约 2 秒,先在右上角画 `OFF` 终帧,再释放 `BAT_Control` 断电
...
开机 NTP:
  ...
  v
loop:
  - 长按 PWR 约 2 秒 -> 画 OFF 终帧 -> 释放 BAT_Control 关机
...
- 关机流程:关 WiFi -> `render_home_powered_off()` 右上角画 `OFF` -> 关屏幕电源 -> 拉低 GPIO17 `BAT_Control`
...
- [ ] 正常开机后,长按 PWR 约 2 秒
- [ ] 屏幕右上角出现 `OFF` 小标记
- [ ] 纯电池供电时,松手后设备停止刷新,最后画面保持在 `OFF` 状态
